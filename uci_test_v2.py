...
from json2vec_v2 import JSONTreeLSTM#, SetJSONNN
import torch
...
def run_once(dataset="car", model_type='json-nn', seed=123, epochs=None, lr=0.000125,
    ...
    if model_type == 'json-nn':
        embedder = JSONTreeLSTM(mem_dim*2, batch=True)#, tie_weights_containers,
    ...
    elif model_type == 'json-nn-modified':
        embedder = JSONTreeLSTM(mem_dim*2, overrides={
                'string~..': 'catEmbedding',
                'number~..': 'catEmbedding',
                '.*': 'deepSets'
            }, batch=True)
    ...
    def test():
        ...
        for i in tqdm.tqdm(range(0,len(X_test), batch_size),
            total=len(X_test)//batch_size, leave=False):

            # get the inputs
            inputs = torch.Tensor(X_test[i:i+batch_size]) if model_type=='mlp' else X_test[i:i+batch_size]
            labels = torch.Tensor([y_test[i:i+batch_size]]) if regression else torch.LongTensor(y_test[i:i+batch_size]) 
            ...
            if not regression:
                ...
                running_test_acc += acc.detach().numpy().sum()

            # print statistics
            running_test_loss += loss.detach().numpy().sum()
    ...
    if True:#poker_frac_test is None:
        l, a = test()
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        ...
        for i in tqdm.tqdm(range(0,len(X_train), batch_size), total=len(X_train)//batch_size,
            leave=False, desc='Epoch {:d}: '.format(epoch)):

            # get the inputs
            inputs = torch.Tensor(X_train[i:i+batch_size]) if model_type=='mlp' else X_train[i:i+batch_size]
            labels = torch.Tensor([y_train[i:i+batch_size]]) if regression else torch.LongTensor(y_train[i:i+batch_size]) 
            ...
            if not regression:
                _, predicted = torch.max(outputs, 1)
                acc = (predicted == labels)
                running_acc += acc.detach().numpy().sum()
                
                running_loss += loss.detach().numpy().sum()# * batch_size
...
if __name__ == "__main__":
    print(run_once(model_type='json-nn', dataset='car', batch_size=4,
        lr=0.001, epochs=20, poker_frac_test=None, test=True))
